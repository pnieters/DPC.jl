...
ax_input = fig[1,2] = Axis(fig; yticks, ytickformat, title = "Input spike volleys")
ax_left  = fig[1:4,1] = Axis(fig; backgroundcolor=:transparent)
ax_right = fig[1:4,3] = Axis(fig; backgroundcolor=:transparent)
ax_chain = fig[2,2] = Axis(fig; yticks, ytickformat, title = "Sequential segments")
ax_or    = fig[3,2] = Axis(fig; yticks, ytickformat, title = "Parallel segments (either required)")
ax_and   = fig[4,2] = Axis(fig; yticks, ytickformat, title = "Parallel segments (both required)")
...
offset = Dict(:chain => Point2f0(0,-2.5), :and => Point2f0(0,-4) , :or => Point2f0(0,-6))
...
for (name, config, ax) in ((:chain,config_chain,ax_chain), (:or,config_or,ax_or), (:and,config_and,ax_and))
    ...
    steps!(ax, [0;n.t;500], 0 .+ 4.9 .* [0;Int.(n.state .== ADSP.voltage_elevated);0], fill=color_3_25, color=:transparent)
    steps!(ax, [0;n.t;500], 0 .+ 4.9 .* [0;Int.(n.state .== ADSP.voltage_high);0], fill=color_3_50, color=:transparent)
...
ylims!(ax_left, (-6.1,2.1))
ylims!(ax_right, (-6.1,2.1))
xlims!(ax_left, (-0.5,0.5))
xlims!(ax_right, (-0.5,0.5))
text!(ax_input, "a", position = (250, 2.5), align = (:center, :center), space=:screen)
